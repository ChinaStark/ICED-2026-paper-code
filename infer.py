...
def get_input_seq(data: Dict[str, Any], prompt_template: str, metadata: Dict[str, str] | None = None) -> str:
    if "{{ERROR}}" in prompt_template:
        prompt = (
            prompt_template
            .replace("{{SCHEMA}}", data.get('create_format') if metadata is None else metadata[data['db_id']])
        ...
        prompt = (
            prompt_template
            .replace("{{SCHEMA}}", data.get('schema') if metadata is None else metadata[data['db_id']])
...
def main(opt):
    ...
    # Stop tokens for typical models
    ...
    if "Qwen3-" in opt.nl2sql_ckpt_path:
        ...
        for i, data in enumerate(input_dataset):
            # Step 1: Build prompt (based on prompt_key)
            ...
            # Step 2: Check token length and fallback to mix_data if too long
