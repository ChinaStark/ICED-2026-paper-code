...
from new_code.utils.bridge_content_encoder import get_matched_entries
from new_code.utils.db_utils import get_db_schema
...
def spider_style_dataset(
    ...
    # TODO 杩欓噷鐢ㄧ殑鏄?column_name鍋氫负 comments
    ...
    db_id2searcher = dict()
    for db_id in db_ids:
        print(os.path.join(db_content_index_path, db_id))
    ...
    for data in dataset:
        ...
        if mode in ["train", "dev"]:
            ...
            sql_for_token = remove_table_alias(sqlparse.format(sql, keyword_case = "upper", identifier_case = "lower"))
            sample["sql"] = sql
        elif mode == "test":
            sample["sql"] = ""
        ...
        try:
            sql_tokens = [token.value for token in Parser(sql_for_token.lower()).tokens]
        except Exception as e:
            sql_tokens = sql_for_token.lower().split()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate BIRD-with-evidence train dataset.")

    # 璺緞绫诲弬鏁?    parser.add_argument("--dataset_path", required=True,
                        help="Path to train.json")
    parser.add_argument("--db_path", required=True,
                        help="Path to train_databases directory")
    parser.add_argument("--db_content_index_path", required=True,
                        help="Path to db_contents_index directory")
    parser.add_argument("--table_json_path", required=True,
                        help="Path to train_tables.json")

    # 杈撳嚭鏂囦欢
    parser.add_argument("--output_json", required=True,
                        help="Where to save the generated SFT JSON file")

    # 甯冨皵寮€鍏?    parser.add_argument("--use_evidence", action="store_true", default=True,
                        help="Whether to use evidence (default: True)")
    parser.add_argument("--source", default="bird-train",
                        help="Dataset source tag (default: bird-train)")
    parser.add_argument("--mode", default="train",
                        help="Mode tag (default: train)")
    ...
    print(f"鉁?Generated {args.output_json}")
