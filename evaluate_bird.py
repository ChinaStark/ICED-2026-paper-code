# -- coding: utf-8 --
# @Author : ZhiliangLong
# @File : evaluate_bird.py
# @Time : 2025/10/23 16:37
import sys
import sqlite3
import json
import argparse
...
def evaluate_sqls_parallel(db_files, questions, pred_sqls, ground_truth_sqls, num_cpus=1, timeout=1):
    ...
    for question_id, db_file, question, pred_sql, ground_truth in zip([x for x in range(len(db_files))], db_files,
                                                                      questions, pred_sqls, ground_truth_sqls):
        pool.apply_async(compare_sql_wrapper, args=((question_id, db_file, question, ground_truth, pred_sql), timeout),
...
def mark_invalid_sqls(db_files, sqls):
    ...
    execution_results = []
    execute_sqls_parallel(db_files, sqls, num_cpus=60, timeout=10)
...
def major_voting(db_files, pred_sqls, sampling_num, return_random_one_when_all_errors=True):
    ...
    execute_sqls_parallel(db_files, pred_sqls, num_cpus=60, timeout=30)
    execution_results = sorted(execution_results, key=lambda x: x['data_idx'])
...
def run_eval(gold_file, pred_file, db_path, mode, save_pred_sqls, num_cpus=60, timeout=30):
    ...
    try:
        ...
        questions = [data["SpiderSynQuestion"] for data in gold]
    pred_sql_key = "pred_sql"
    # pred_sql_key = "responses"

    # if "spider" in  gold_file:
    ...
    if mode == "greedy_search":
        ...
        print(len(pred_sqls), len(db_files))
        ...
        for sample_idx in range(sampling_num):
            ...
            evaluation_results = []
            evaluate_sqls_parallel(db_files, questions, pred_sqls_for_specific_sample_idx, ground_truth_sqls,
...
python evaluate_bird.py --pred ./results/Qwen2.5-Coder-7B-Instruct_dev_bird/sampling_.json --gold ../data/bird_dataset/dev_20240627/dev.json --db_path ../data/bird_dataset/dev_20240627/dev_databases/ --mode major_voting
'''

if __name__ == "__main__":
    opt = parse_option()
    run_eval(opt.gold, opt.pred, opt.db_path, opt.mode, True)